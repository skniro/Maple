...
import com.skniro.maple.mixin.qsldatafixupper.misc.DataFixTypesAccessor;
import com.skniro.maple.mixin.qsldatafixupper.misc.NbtHelperMixin;
...
public final class QuiltDataFixesInternalsImpl extends QuiltDataFixesInternals {
	...
	@Override
	public @NotNull Dynamic<NbtElement> updateWithAllFixers(@NotNull DataFixTypes dataFixTypes, @NotNull Dynamic<NbtElement> current) {
		NbtCompound compound = (NbtCompound)current.getValue();

		int modDataVersion;
		QuiltDataFixesInternals.DataFixerEntry dataFixerEntry;
		for(Iterator var4 = this.modDataFixers.entrySet().iterator(); var4.hasNext(); current = dataFixerEntry.dataFixer().update((DataFixTypesAccessor.class.cast(dataFixTypes)).getTypeReference(), current, modDataVersion, dataFixerEntry.currentVersion())) {
			Map.Entry<String, QuiltDataFixesInternals.DataFixerEntry> entry = (Map.Entry)var4.next();
			modDataVersion = getModDataVersion(compound, (String)entry.getKey());
			dataFixerEntry = (QuiltDataFixesInternals.DataFixerEntry)entry.getValue();
		}
