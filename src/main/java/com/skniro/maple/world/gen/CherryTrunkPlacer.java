...
public class CherryTrunkPlacer extends TrunkPlacer {
    ...
    public List<FoliagePlacer.TreeNode> generate(TestableWorld world, BiConsumer<BlockPos, BlockState> replacer, Random random, int height, BlockPos startPos, TreeFeatureConfig config) {
        ...
        Function<BlockState, BlockState> function = (state) -> {
            return (BlockState)state.with(PillarBlock.AXIS, direction.getAxis());
        };
    ...
    public static <T extends IntProvider> Codec<T> createValidatingCodec(int min, int max, Codec<T> providerCodec) {
        return validate(providerCodec, (provider) -> {
    ...
    static {
        BRANCH_START_OFFSET_FROM_TOP_CODEC = validate(UniformIntProvider.CODEC, (branchStartOffsetFromTop) -> {
            return branchStartOffsetFromTop.getMax() - branchStartOffsetFromTop.getMin() < 1 ? DataResult.error("Need at least 2 blocks variation for the branch starts to fit both branches") : DataResult.success(branchStartOffsetFromTop);
        });
        CODEC = RecordCodecBuilder.create((instance) -> {
            ...
            }), createValidatingCodec(-16, 0, BRANCH_START_OFFSET_FROM_TOP_CODEC).fieldOf("branch_start_offset_from_top").forGetter((trunkPlacer) -> {
