...
public class MapleHotSpringFluid extends FlowableFluid {
    ...
    public void randomDisplayTick(World world, BlockPos pos, FluidState state, Random random) {
        ...
        if (random.nextFloat() < 0.07F) {
            world.addParticleClient(MapleParticleTypes.HOT_SPRING, (double) pos.getX() + random.nextDouble(), (double) pos.getY() + random.nextDouble(), (double) pos.getZ() + random.nextDouble(), 0.0, 0.03, 0.0);
        }
    }
    ...
    protected BlockState toBlockState(FluidState state) {
        return MapleFluidBlockOrItem.Hot_Spring_BLOCK.getDefaultState().with(Properties.LEVEL_15, getBlockStateLevel(state));
    }
